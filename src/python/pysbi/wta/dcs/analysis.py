...
class SubjectReport:
    ...
    def plot_incongruent_correct_bias_rt(self, furl, dt, colors):
        ...
        save_to_png(fig, '%s.png' % fname)
        save_to_eps(fig, '%s.eps' % fname)
        plt.close(fig)def plot_incongruent_correct_bias_rt(self, furl, dt, colors):
        fname=os.path.join(self.report_dir, furl)
        condition_biases={}
        condition_rts={}
        for stim_level, session_report in self.sessions.iteritems():
...
#        self.large_input_diff_accuracy_logistic_coeffs={'bias':{},'ev diff':{}}
    ...
    def create_report(self, regenerate_plots=True, regenerate_subject_plots=True, regenerate_session_plots=True,
        ...
        furl='img/congruent_bias_rt'
        self.congruent_bias_rt_url='%s.png' % furl
        if regenerate_plots:
            self.plot_congruent_bias_rt(furl, self.dt, condition_colors)

        furl='img/incongruent_bias_rt'
        self.incongruent_bias_rt_url='%s.png' % furl
        if regenerate_plots:
            self.plot_incongruent_bias_rt(furl, self.dt, condition_colors)
