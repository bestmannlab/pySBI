...
class StimConditionReport:
    ...
    def create_report(self):
        ...
        if not os.path.exists('%s.png' % fname):
            fig=Figure()
            ax=fig.add_subplot(1,1,1)
            ax.plot(np.mean(self.small_beta_small_ev_diff_chosen_rates,axis=0),'b',label='small beta, chosen')
            ax.plot(np.mean(self.small_beta_small_ev_diff_unchosen_rates,axis=0),'b--',label='small beta, unchosen')
            ax.plot(np.mean(self.med_beta_small_ev_diff_chosen_rates,axis=0),'g',label='med beta, chosen')
            ax.plot(np.mean(self.med_beta_small_ev_diff_unchosen_rates,axis=0),'g--',label='med beta, unchosen')
            ax.plot(np.mean(self.large_beta_small_ev_diff_chosen_rates,axis=0),'r',label='large beta, chosen')
            ax.plot(np.mean(self.large_beta_small_ev_diff_unchosen_rates,axis=0),'r--',label='large beta, unchosen')
            ax.set_xlabel('Time')
            ax.set_ylabel('Firing Rate (Hz)')
            ax.legend(loc='best')
            save_to_png(fig, '%s.png' % fname)
        ...
        if not os.path.exists('%s.png' % fname):
            fig=Figure()
            ax=fig.add_subplot(1,1,1)
            ax.plot(np.mean(self.small_beta_med_ev_diff_chosen_rates,axis=0),'b',label='small beta, chosen')
            ax.plot(np.mean(self.small_beta_med_ev_diff_unchosen_rates,axis=0),'b--',label='small beta, unchosen')
            ax.plot(np.mean(self.med_beta_med_ev_diff_chosen_rates,axis=0),'g',label='med beta, chosen')
            ax.plot(np.mean(self.med_beta_med_ev_diff_unchosen_rates,axis=0),'g--',label='med beta, unchosen')
            ax.plot(np.mean(self.large_beta_med_ev_diff_chosen_rates,axis=0),'r',label='large beta, chosen')
            ax.plot(np.mean(self.large_beta_med_ev_diff_unchosen_rates,axis=0),'r--',label='large beta, unchosen')
            ax.set_xlabel('Time')
            ax.set_ylabel('Firing Rate (Hz)')
            ax.legend(loc='best')
            save_to_png(fig, '%s.png' % fname)
        ...
        if not os.path.exists('%s.png' % fname):
            fig=Figure()
            ax=fig.add_subplot(1,1,1)
            ax.plot(np.mean(self.small_beta_large_ev_diff_chosen_rates,axis=0),'b',label='small beta, chosen')
            ax.plot(np.mean(self.small_beta_large_ev_diff_unchosen_rates,axis=0),'b--',label='small beta, unchosen')
            ax.plot(np.mean(self.med_beta_large_ev_diff_chosen_rates,axis=0),'g',label='med beta, chosen')
            ax.plot(np.mean(self.med_beta_large_ev_diff_unchosen_rates,axis=0),'g--',label='med beta, unchosen')
            ax.plot(np.mean(self.large_beta_large_ev_diff_chosen_rates,axis=0),'r',label='large beta, chosen')
            ax.plot(np.mean(self.large_beta_large_ev_diff_unchosen_rates,axis=0),'r--',label='large beta, unchosen')
            ax.set_xlabel('Time')
            ax.set_ylabel('Firing Rate (Hz)')
            ax.legend(loc='best')
            save_to_png(fig, '%s.png' % fname)
...
class RLReport:
    ...
    def create_report(self):
        ...
        if not os.path.exists('%s.png' % fname):
            ...
            ax.set_xticks(pos, self.stim_conditions)
            save_to_png(fig, '%s.png' % fname)
            save_to_eps(fig, '%s.eps' % fname)
            plt.close(fig)
